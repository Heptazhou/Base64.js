...
if (!btoa) {
	...
	utf8Base64Decode = (base64Str) => {
		...
		while (i < length) {
			...
			if (b <= 0x7f) {
				...
				c = b & 0x07
				followingChars = 3
			} else throw "not UTF-8"

			for (var j = 0; j < followingChars; ++j) {
				b = bytes[i++]
				if (b < 0x80 || b > 0xbf) throw "not UTF-8"
				c <<= 6
				c += b & 0x3f
			}
			if ((c >= 0xd800 && c <= 0xdfff) || c > 0x10ffff) throw "not UTF-8"
	...
	utf8Base64Decode = (base64Str) => {
		...
		if (!/[^\x00-\x7F]/.test(tmpStr)) return tmpStr
		var str = "",
			i = 0,
...
export const encode = (str, asciiOnly) => {
...
export const decode = (base64Str, asciiOnly, mode = "string") => {
	if (mode === "bytes") return decodeAsBytes(base64Str)
	else return asciiOnly ? atob(base64Str) : utf8Base64Decode(base64Str)
}
